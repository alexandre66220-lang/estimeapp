...
export default function MentionsLegales() {
  return (
    ...
    >
      <div className="space-y-12 text-charbon">

        {SECTIONS.map((section) => (
          <section key={section.title}>
            <h2 className="font-display text-xl font-bold text-charbon mb-5 pb-3 border-b border-charbon/10">
              {section.title}
            </h2>
            <dl className="space-y-3">
              {(section.content as [string, string][]).map(([label, value]) => (
                <div key={label} className="grid grid-cols-[180px_1fr] gap-4">
                  <dt className="text-sm text-charbon/50 font-medium">{label}</dt>
                  <dd className="text-sm text-charbon/80">{value}</dd>
                </div>
              ))}
            </dl>
          </section>
        ...
        <section>
          ...
          <div className="space-y-3 text-sm text-charbon/70 leading-relaxed">
            <p>
